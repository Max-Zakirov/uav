#include "CRSF.h"
#include <iostream>

/* CRC8 implementation with polynom = x7+ x6+ x4+ x2+ x0 (0xD5) */
unsigned char crc8tab[256] = {
    
};

uint8_t crc8(const uint8_t * ptr, uint8_t len) {
    uint8_t crc = 0;
    for (uint8_t i=0; i<len; i++) {
        crc = crc8tab[crc ^ *ptr++];
    }
    return crc;
}

CRSF::CRSF() {
    channels.resize(16, 992); // Initialize all channels to neutral value

    // Define key mappings for control inputs
    keyMap['w'] = 0;  // Throttle (Channel 0)
    keyMap['s'] = 0;
    keyMap['a'] = 1;  // Roll (Channel 1)
    keyMap['d'] = 1;
}

std::array<uint8_t, CRSF_SIZE> CRSF::unpack() {
    std::array<uint8_t, CRSF_SIZE> packet{};

    packet[0] = CRSF_RC_PACKET;
    packet[1] = CRSF_PAYLOAD_SIZE;
    packet[2] = CRSF_RC_PACKET;

    uint8_t* payload = &packet[3];
    uint32_t bitBuffer = 0;
    uint8_t bitCount = 0;
    int payloadIndex = 0;

    for (int i = 0; i < 16; i++) {
        bitBuffer |= (channels[i] & 0x07FF) << bitCount;
        bitCount += 11;

        while (bitCount >= 8) {
            payload[payloadIndex++] = bitBuffer & 0xFF;
            bitBuffer >>= 8;
            bitCount -= 8;
        }
    }

    if (bitCount > 0) {
        payload[payloadIndex++] = bitBuffer & 0xFF;
    }

    packet[25] = crc8(&packet[2], 23);

    return packet;
}

void CRSF::mapKeyToChannel(char key) {
    switch (key) {
        /* ROLL */
        case 'a': channels[0] -= 50; break;
        case 'd': channels[0] += 50; break;
        /* PITCH*/
        case 'w': channels[1] += 50; break;
        case 's': channels[1] -= 50; break;
        /* TROTTLE */
        case 'r': channels[2] -= 10;  break;
        case 'f': channels[2] += 10;  break;
        /* YAW */
        case 'q': channels[3] -= 10;  break;
        case 'e': channels[3] += 10;  break;
        /* ARM */
        case 'z': channels[4] = 580;  break;
        case 'x': channels[4] = 1600; break;
        case 'c': channels[2] = 120;  break;
        default:  std::cout << "Invalid key: " << key << std::endl; return;
    }
}

std::vector<uint16_t> CRSF::getChannels() const {
    return channels;
}
